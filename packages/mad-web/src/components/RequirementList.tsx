...
export function RequirementList({ requirements, showStatus, showTechnical }: RequirementListProps) {
  const { t, statusLabel, severityLabel } = useLocale();
  ...
  return (
    <ul className="req-list">
          ...
          <li key={id} className="req-item">
            ...
            {showTechnical && "evaluation_method" in req && req.evaluation_method && (
              <div className="req-tech">
                <strong>{t.requirementList.evaluationMethod}</strong> {req.evaluation_method}
              </div>
            )}
            {showTechnical && "technical_criteria" in req && req.technical_criteria && (
              <div className="req-tech">
                <strong>{t.requirementList.technicalCriteria}</strong> {req.technical_criteria}
              </div>
            )}
      ...
      <style>{`
        ...
        .req-item { padding: 1rem; border-bottom: 1px solid #e2e6ea; }
        ...
        .req-title { margin: 0.4rem 0 0; font-weight: 600; font-size: 0.95rem; }
        .req-desc { margin: 0.25rem 0 0; font-size: 0.85rem; color: var(--mad-text-muted); }
        .req-tech {
          margin: 0.5rem 0 0; font-size: 0.8rem; color: var(--mad-text-muted);
          line-height: 1.5; padding: 0.5rem 0.75rem; background: #f4f6f8; border-radius: 4px;
        }
        .req-tech strong {
          color: var(--mad-navy); font-size: 0.75rem;
          text-transform: uppercase; letter-spacing: 0.03em;
        }
        .req-notes {
          margin: 0.5rem 0 0; font-size: 0.8rem; font-style: italic;
          color: var(--mad-high); padding-left: 0.75rem; border-left: 3px solid var(--mad-high);
        }
      `}</style>
    </ul>
  );
